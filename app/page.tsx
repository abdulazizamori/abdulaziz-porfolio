...
export default function Home() {
  ...
  return <main onPointerMove={followPointer}>
    ...
    <section id="projects" className="section projects"><Heading num="03" label="SELECTED WORK" title={<>Products with real<br/>operational weight.</>}/><div className="project-list">{projects.map((project, index) => <ProjectCard key={`project-${project.slug}-${index}`} project={project} index={index}/>)}</div></section>
    ...
    <section id="contact" className="section contact"><Heading num="07" label="CONTACT" title={<>Have a product idea?<br/><i>Let’s build it.</i></>}/><div className="contact-grid"><div><p>Open to product, freelance, and collaborative opportunities. Tell me what you’re working on.</p><a href="mailto:abdulaziz.amori10@gmail.com">abdulaziz.amori10@gmail.com <ArrowUpRight size={18}/></a></div><form action="https://formsubmit.co/abdulaziz.amori10@gmail.com" method="POST"><input type="hidden" name="_subject" value="New portfolio contact message"/><input type="hidden" name="_template" value="table"/><input required name="name" placeholder="Your name"/><input required name="email" type="email" placeholder="Email address"/><input name="company" placeholder="Company (optional)"/><textarea required name="message" placeholder="What would you like to build?" rows={5}/><button className="button" type="submit">Send message <Send size={16}/></button></form></div></section>
    <footer>© 2026 Abdulaziz Amori <span>·</span> Full-Stack & AI Engineer</footer>
  </main>;
}
function Heading({num,label,title}:{num:string;label:string;title:React.ReactNode}) { return <motion.div {...reveal} className="heading"><p>{num} / {label}</p><h2>{title}</h2></motion.div> }
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  return <motion.article {...reveal} className="project" style={{ '--accent': project.accent } as React.CSSProperties}>
    <div className="project-info">
      <p className="eyebrow">0{index + 1} / {project.kind}</p><h3>{project.name}</h3><p>{project.summary}</p><small>{project.role}</small>
